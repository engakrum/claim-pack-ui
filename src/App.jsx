  ...
  const runGeneration = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("Server Error. The backend might still be waking up. Wait 30s and try again.");
      }
    } catch (error) {
      alert("Network Error: Backend is unreachable. Check Render Dashboard.");
    } finally {
      ...
      {showPaywall && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#0B1121] border border-teal-500 rounded-2xl p-8 max-w-md w-full shadow-2xl text-center">
            ...
            <button onClick={() => alert("Stripe Payment Integration")} className="w-full py-3 rounded-xl bg-teal-500 hover:bg-teal-400 text-[#020617] font-bold mb-3">
              Upgrade for $9.99
            </button>
...
const LandingPage = ({ onGetStarted }) => {
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100 font-sans selection:bg-teal-500/30">
      ...
      {/* HERO SECTION - MATCHING YOUR SCREENSHOT */}
      <div className="container mx-auto px-6 pt-20 pb-20 text-center max-w-5xl">
        ...
        <div className="flex justify-center gap-4">
           <button onClick={onGetStarted} className="h-12 px-8 rounded-xl bg-teal-500 hover:bg-teal-400 text-[#020617] font-bold text-lg flex items-center gap-2 shadow-xl shadow-teal-500/20 hover:scale-105 transition-all">
             Start Building Your Case <ArrowRight className="w-5 h-5" />
           </button>
           <button className="h-12 px-8 rounded-xl border border-slate-700 hover:bg-slate-900 text-white font-medium transition-all">Watch Demo</button>
        </div>
      </div>

      {/* PRICING SECTION - LOGIC: $0 vs $9.99 */}
